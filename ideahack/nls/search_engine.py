...
class HybridSearchSystem:
    def __init__(self, filters_file: str, vector_store_handler: VectorStoreHandler, profile_store_handler: ProfileStoreHandler, sentence_model: SentenceTransformer):
    ...
    def parse_filters_and_classify_profiles(self, user_query):
        ...
        filter_values = "\n".join([f"{key}: {', '.join(values)}" for key, values in filter_types.items()])
    ...
    def filter_profiles(self, filters, profile_types):
        ...
        if "USER" in profile_types:
            ...
            user_related_filters = { # key - field in db, value - filter values
                "bio": filters.get("TECHNOLOGY", []) + filters.get("INDUSTRY", []) + filters.get("ROLE", []) + filters.get("EXPERIENCE", []),
                "experience": filters.get("TECHNOLOGY", []) + filters.get("INDUSTRY", []) + filters.get("ROLE", []) + filters.get("EXPERIENCE", []),
            ...
            for field, values in user_related_filters.items():
                for value in values:
                    self.profile_store_handler.cursor.execute(f"SELECT vector_id FROM User WHERE {field} LIKE ?", (f"%{value}%",))
        ...
        if "COMPANY" in profile_types:
            ...
            for field, values in company_related_filters.items():
                for value in values:
                    self.profile_store_handler.cursor.execute(f"SELECT vector_id FROM Company WHERE {field} LIKE ?", (f"%{value}%",))
                    rows = self.profile_store_handler.cursor.fetchall()
                    filtered_ids.extend([row[0] for row in rows])
    ...
    def hybrid_search(self, user_query):
        ...
        query_embedding = self.sentence_model.encode(user_query, convert_to_tensor=False)
        result_ids = self.vector_store_handler.search_vectors(query_embedding, filtered_ids, top_k=5)
