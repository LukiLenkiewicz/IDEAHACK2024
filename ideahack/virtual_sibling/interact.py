...
class VirtualSibling:
    ...
    def _get_profile_data(self, db_path):
        ...
        if self.profile_type == 'user':
            cursor.execute('''SELECT name, description, experience, skills, website, social_media FROM user_profiles WHERE user_id = ?''', (self.profile_id,))
        elif self.profile_type == 'company':
            cursor.execute('''SELECT name, description, services, experience, industry_keywords, website, social_media FROM company_profiles WHERE company_id = ?''', (self.profile_id,))
        
        profile_data = cursor.fetchone()
        conn.close()
        
        if profile_data:
            keys = ['name', 'description', 'experience', 'skills', 'website', 'social_media'] if self.profile_type == 'user' else ['name', 'description', 'services', 'experience', 'industry_keywords', 'website', 'social_media']
    ...
    def query(self, user_query):
        ...
        details = [self.profile_data[key] for key in self.profile_data if key not in ['name', 'description']]
