...
from .models import (
    ...
    History,
    UserGenre,
    AlgorithmRecomendations,
    GenreShort,
    OFFTOP_Recomendations,
    Review,
)
...
def getBook(request, pk):
    ...
    data_review = Review.objects.filter(Reader_ID=user_obj, Book_ID=book).values_list(
        "Review", flat=True
    )
...
def updateUser(request, id):
    ...
    BookReaders.objects.filter(FireBaseAuth=id).update(
        Name=data_user["name"], Age=data_user["age"], Sex=data_user["sex"]
    )
...
def registerUser(request):
    ...
    genres_id = GenreShort.objects.filter(
        Genre_Name__in=data_user["genres"]
    ).values_list("Genre_ID", flat=True)
    user_genres_types = [
        UserGenre(
            Reader_ID=new_user, Genre_ID=GenreShort.objects.get(Genre_ID=genre_id)
        )
...
def setUserHistory(request):
    ...
    if book.Availability != 0:
        is_this_book_rentend = History.objects.filter(
            Reader_ID=user.Reader_ID, Book_ID=book.Book_ID
        ).values_list("Returned", flat=True)
        if not (False in is_this_book_rentend):
            History.objects.create(
                Reader_ID=user,
                Book_ID=book,
            )
            Book.objects.filter(Book_ID=data_user["book_id"]).update(
                Availability=book.Availability - 1
            )

            return Response("History added")
        return Response(
            "Failed to add into history - already taken",
...
def updateUserGenre(request):
    ...
    genres_id = GenreShort.objects.filter(
        Genre_Name__in=data_user["new_genres"]
    ).values_list("Genre_ID", flat=True)
    user_genres_types = [
        UserGenre(Reader_ID=user, Genre_ID=GenreShort.objects.get(Genre_ID=genre_id))
        for genre_id in genres_id
    ]
    UserGenre.objects.bulk_create(user_genres_types)
...
def getUserDataAlgo(request, id):
    ...
    if numberHistory < 4:
        ...
        user_recomendation = AlgorithmRecomendations.objects.filter(
...
def addReview(request):
    ...
    review_counter = Review.objects.filter(
        Reader_ID=user_obj, Book_ID=book_obj
    ...
    if len(review_counter) == 0:
        ...
        book_obj.Rating = round(
            (book_obj.Rating * book_obj.NumberReviews + rating)
            / (book_obj.NumberReviews + 1),
            2,
        )
        ...
        Review.objects.filter(Reader_ID=user_obj, Book_ID=book_obj).update(
            Review=rating
        )
        book_obj.Rating = round(
            (book_obj.Rating * book_obj.NumberReviews - review_counter[0] + rating)
            / (book_obj.NumberReviews),
            2,
        )
        book_obj.save()
